...
function computeModel(sim, params) {
  ...
  const housingCost = zeros(), loanBalance = zeros(), loanInterest = zeros();
  const buildingVal = zeros(), landVal = zeros(), saleEstimate = zeros(), realEstateAsset = zeros();
...
function StatCard({ label, value, tone = "ink", small }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: small ? 16 : 20, fontWeight: 700, color, fontVariantNumeric: "tabular-nums", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{value}</div>
    </div>
  );
}

function NumField({ value, onChange, width = 74, suffix }) {
  return (
    <div style={{ position: "relative" }}>
      <input
        ...
        style={{
          ...
          borderRadius: 3, fontVariantNumeric: "tabular-nums", color: INK, background: "#FFFDF9",
        }}
      />
    </div>
  );
      ...
      <div className="sheet-print-target" style={{ padding: "0 16px 16px" }}>
        <div className="sheet-scroll-container" style={{ overflow: "auto", border: `1px solid ${PAPER_LINE}`, borderRadius: 4, maxHeight: "70vh" }}>
          <div style={{ minWidth: 128 + N * 82 }}>
            ...
            <SheetSectionLabel text={`住宅（${HOUSING_LABELS[params.housingType]}）`} />
...
function SimulationTab({ sim, setSim, params, setParams }) {
  ...
  return (
    <div style={{ paddingBottom: 40 }}>
      <SectionHeader title="資産推移シミュレーション" sub="前提条件を編集すると、下のグラフが即座に再計算されます" />
      ...
      <div style={{ padding: "8px 16px 0", height: 210, background: CARD, marginBottom: 18 }}>
        <ResponsiveContainer width="100%" height="100%">
      ...
      <div style={{ padding: "0 16px 16px", display: "flex", flexDirection: "column", gap: 10 }}>
        <div style={{ background: CARD, border: `1px solid ${PAPER_LINE}`, borderRadius: 5, padding: 12 }}>
          ...
          {params.housingType === 1 && (
            <label style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 10, fontSize: 12.5, color: INK_SOFT }}>
      ...
      <div style={{ padding: "0 16px 8px" }}>
        <Accordion title="収入" colorKey="tuition" defaultOpen>
          ...
          <div style={{ fontSize: 11.5, color: INK_SOFT, marginTop: 8 }}>配当収入（前年金融資産×配当利回り）は自動計算されます</div>
        </Accordion>
      </div>
    </div>
  );
...
function ParamField({ label, value, suffix, onChange }) {
  return (
    <div style={{ background: CARD, border: `1px solid ${PAPER_LINE}`, borderRadius: 5, padding: "8px 12px", flex: "1 1 140px" }}>
      <div style={{ fontSize: 11, color: INK_SOFT, marginBottom: 3 }}>{label}</div>
      <div style={{ display: "flex", alignItems: "baseline", gap: 4 }}>
        <input type="number" value={value} step="0.1"
          onChange={(e) => onChange(e.target.value === "" ? 0 : parseFloat(e.target.value))}
          style={{ width: 70, fontSize: 15, fontWeight: 700, color: INK, border: "none", borderBottom: `1px solid ${PAPER_LINE}`, fontVariantNumeric: "tabular-nums" }} />
...
Include up to 3 plausible candidates, best match first. If nothing plausible is found, return {"candidates":[]}.`;
  ...
  const isFundPicked = picked && (picked.instrumentType === "投信" || !picked.exchange);
...
function GoogleSheetEmbed() {
  ...
  const toEmbedUrl = (raw) => {
    // 「ウェブに公開」した pubhtml リンクをそのまま埋め込みURLとして使う
    // 通常のスプレッドシート編集URLの場合は自動で pubhtml 形式に変換を試みる
    if (!raw) return "";
    if (raw.includes("/pubhtml") || raw.includes("output=html")) return raw;
  ...
  return (
    <div style={{ background: CARD, border: `1px solid ${PAPER_LINE}`, borderRadius: 5, padding: 12, marginBottom: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ fontSize: 12.5, fontWeight: 600, color: INK }}>Googleスプレッドシートを埋め込む（試験的）</div>
        <button onClick={() => setShowInput((s) => !s)} style={{ fontSize: 11, color: INK_SOFT, background: "transparent", border: "none", cursor: "pointer" }}>
          {showInput ? "閉じる" : "設定"}
        </button>
      </div>
      {showInput && (
        <div style={{ marginTop: 8 }}>
          <div style={{ fontSize: 10.5, color: INK_SOFT, marginBottom: 6 }}>
            スプレッドシートを「ファイル→共有→ウェブに公開」した上で、そのURL（または通常の共有URL）を貼り付けてください。
          </div>
          <div style={{ display: "flex", gap: 6 }}>
            <input value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://docs.google.com/spreadsheets/d/..."
              style={{ flex: 1, fontSize: 12, padding: "6px 8px", border: `1px solid ${PAPER_LINE}`, borderRadius: 4 }} />
            <button onClick={() => { setApplied(toEmbedUrl(url)); setIframeOk(true); }} style={{
              fontSize: 12, padding: "6px 12px", borderRadius: 4, border: "none", background: GOLD, color: "#fff", cursor: "pointer",
            }}>表示</button>
          </div>
        </div>
      )}
          ...
          {iframeOk ? (
            <div style={{ position: "relative" }}>
              <iframe
                ...
                style={{ width: "100%", height: 340, border: `1px solid ${PAPER_LINE}`, borderRadius: 4 }}
              />
              <div style={{ fontSize: 10, color: INK_SOFT, marginTop: 4 }}>
                何も表示されない場合は、このアプリのサンドボックス上の制限で埋め込みがブロックされています。下のリンクから別タブで開いてください。
              </div>
            </div>
          ) : null}
          <a href={url} target="_blank" rel="noopener noreferrer" style={{
            display: "inline-block", marginTop: 8, fontSize: 12, color: GOLD, textDecoration: "underline",
          }}>↗ 元のGoogleスプレッドシートを別タブで開く</a>
        </div>
      )}
    </div>
  );
...
function AggregationTab({ holdings, cashList, sim, params, asOfDate, yearSnapshots, setYearSnapshots }) {
  const totalCash = cashList.reduce((s, c) => s + (c.amount || 0), 0);
  const model = useMemo(() => computeModel(sim, params), [sim, params]);
